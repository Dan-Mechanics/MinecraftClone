...
Texture::Texture(const std::string& image, const std::string& type, GLuint slot) {
	// Assigns the type of the texture ot the texture object
	this->type = type.c_str();

	// Stores the width, height, and the number of color channels of the image
	int widthImg, heightImg, colorChannelCount;
	// Flips the image so it appears right side up

	stbi_set_flip_vertically_on_load(true);

	// Reads the image from a file and stores it in bytes
	unsigned char* bytes = stbi_load(image.c_str(), &widthImg, &heightImg, &colorChannelCount, 0);
