...
void Terraformer::update(GLFWwindow* window, const Raycast& raycast, World & world, ThreadPool& pool, const Atlas& atlas) {
...
void Terraformer::add(const Raycast& raycast, World& world, ThreadPool& pool, const Atlas& atlas) const {
	glm::ivec3 blockPos{};
	glm::ivec3 normal{};
	if (!world.raycast(raycast, blockPos, normal))
		return;
