...
bool fillChunkDataRandom(std::unordered_map<glm::ivec3, Chunk*, ivec3hash>& chunks, const int chunkSize, const glm::ivec3& chunkPos) {
	if (chunkPos.y != 0)
		return false;
		...
		for (int y = 0; y < 2; ++y) {
			for (int z = 0; z < chunkSize; ++z) {
				...
				if (y == 0) {
					...
					chunks[chunkPos]->blocks[blockPos] = static_cast<BlockType>(randomInclusive(0, BlockType::GHOST_LOG));
				}
			}
		}
	}
...
bool fillChunkDataForest(std::unordered_map<glm::ivec3, Chunk*, ivec3hash>& chunks, const int chunkSize, const glm::ivec3& chunkPos) {
	if (chunkPos.y != 0)
		return false;
